...
public class BotStarter {
    ...
    public Move doMove(BotState state) {
        ...
        ArrayList<Point>snippetPos=state.getField().getSnippetPositions();
        ...
        for (int x=0;x<snippetPos.size();x++)
        	
        {if ((me.getBombs() <= 0) && (state.getField().getMyPosition().getX()!=snippetPos.get(x).getX()))
           {
            return new Move(randomMoveType); // No bombs available
           }
        else {if (state.getField().getMyPosition()) ;
        }
